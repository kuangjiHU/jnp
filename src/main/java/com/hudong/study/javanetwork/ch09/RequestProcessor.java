...
public class RequestProcessor implements Runnable {
    ...
    public RequestProcessor(File rootDirectory, String indexFileName, Socket connection) {
        if (!rootDirectory.isDirectory()) {
            throw new IllegalArgumentException(rootDirectory + " does not exist  as a directory");
        }
        try {
    ...
    public void run() {
        ...
        try {
            ...
            StringBuilder rqstLine = new StringBuilder();
            while (true) {
                int c = in.read();
                if (c == '\r' || c == '\n' || c == -1) {
                    break;
                }
                rqstLine.append((char) c);
            }
            String get = rqstLine.toString();
